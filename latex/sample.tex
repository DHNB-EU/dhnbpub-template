...
\conference{Woodstock'23: Symposium on the irreproducible science,
  June 07--11, 2023, Woodstock, NY}

%%
%% The "title" command
\title{A Better Way to Format Your Document for DHNB Publications}

\tnotemark[1]
\tnotetext[1]{You can use this document as the template for preparing your
  publication. We recommend using the latest version of the dhnbpub style.}
...
email=name.surname@university1.edu,
url=https://university1.edu/\~surname,
]
\cormark[1]
\fnmark[1]
...
orcid=0000-0000-0000-0000,
email=name.surname@university2.edu,
url=https://university2.edu/\~surname,
]
\fnmark[1]
%\address[3]{Vrije Universiteit Amsterdam}

\author[3]{Name Surname}[%
orcid=0000-0000-0000-0000,
email=name.surname@university3.edu,
url=https://university3.edu/\~surname,
]
\fnmark[1]
\address[3]{University 3}
...
\begin{abstract}
  ...
  article formatted for publication by DHNB Publications in a conference
  proceedings. Based on the ``dhnbpub'' document class, this article
...
DHNB Publications's article template provides a consistent \LaTeX{} style for
use across DHNB publications, and incorporates accessibility and
metadata-extraction functionality. This document will explain the
major features of the document class.

If you are new to publishing with DHNB Publications, this document is a valuable guide to the process of preparing your work for publication.

The ``\verb|dhnbpub|'' document class can be used to prepare articles
for any DHNB publication, and for any stage of publication, from
...
\begin{itemize}
\item \verb|biblatex-chicago.sty| for citation processing;
...
All the above packages are part of any modern
...
parameters which modify some part of the \verb|dhnbpub| document class.
This parameters are enclosed in square
brackets and are a part of the \verb|\documentclass| command:
\begin{lstlisting}
  \documentclass[parameter]{dhnbpub}
\end{lstlisting}
...
The titles of papers as well as headings should be written in Title Case. Use the \verb|\title| command to define the title of your work. Do not
...
The ``\verb|dhnbpub|'' document class includes the ``\verb|booktabs|''
...
The use of Bib\LaTeX{} for the preparation and formatting of one's
...
commands, one placed at the beginning, and the other just before the \verb|\end{document}|
command:
\begin{lstlisting}
...
suffix, of the Bib\TeX{} file. The citation style used is the Chicago 
Manual of Style, 17th edition author-date.

\subsection{Some Examples}

A paginated journal article \citep{Abril07}, an enumerated journal
article \citep{Cohen07}, a reference to an entire issue
\citep{JCohen96}, a monograph (whole book) \citep{Kosiur01}, a
...
the series if the volume number is given \citep{Editor00a} (so series
should not be present since it has no vol. no.), a chapter in a
divisible book \citep{Spector90}, a chapter in a divisible book in a
series \citep{Douglass98}, a multi-volume work as book \citep{Knuth97},
an article in a proceedings (of a conference, symposium, workshop for
example) (paginated proceedings article) \citep{Andler79}, a
proceedings article with all possible elements \citep{Smith10}, an
example of an enumerated proceedings article \citep{VanGundy07}, an
informally published work \citep{Harel78}, a doctoral dissertation
\citep{Clarkson85}, a master's thesis: \citep{anisi03}, an online
document / world wide web resource \citep{Thornburg01, Ablamowicz07,
  Poker06}, a video game (Case 1) \citep{Obama08} and (Case 2)
\citep{Novak03} and \citep{Lee05} and (Case 3) a patent
\citep{JoeScientist001}, work accepted for publication \citep{Rous08},
prolific author \citep{SaeediMEJ10} and \citep{SaeediJETC10}. Other
cites might contain `duplicate' DOI and URLs (some SIAM articles)
\citep{Kirschmer:2010:AEI:1958016.1958018}. Multi-volume works as books
\citep{MR781536} and \citep{MR781537}. A couple of citations with DOIs:
\citep{2004:ITE:1009386.1010128,Kirschmer:2010:AEI:1958016.1958018}. Online
citations: \citep{TUGInstmem, Thornburg01, R, UMassCitations}. Within text, 
you can refer to a particular author as "\citet{Abril07} saying something".
...
\begin{acknowledgments}
  Thanks to the developers of the CEURART LaTeX styles
  \url{https://github.com/yamadharma/ceurart}.
\end{acknowledgments}
...
The sources for the dhnbpub style are available via
\begin{itemize}
\item \href{https://github.com/DHNB-EU/dhnbpub-template}{GitHub}
